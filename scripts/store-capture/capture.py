...
def run(*args,check=True,env=None):
 p=subprocess.run(args,check=check,capture_output=True,text=True,env=env)
 if p.stderr: print(p.stderr,flush=True)
 return p.stdout.strip()
run('xcrun','simctl','help','io')
...
app=next(pathlib.Path(sys.argv[1]).glob('*.app'))
bundle='com.pawssible.chorely'
provenance={'source_commit':'4858c4c82c06b952340b1ce36f9b77541b1385f3','capture_commit':os.environ.get('GITHUB_SHA'),'xcode':run('xcodebuild','-version'),'runtime':runtime,'method':'Native Capacitor iOS app rendered in Apple iOS Simulator; simctl screenshot. Only authentication and content were replaced with synthetic fixtures. Product UI/CSS unchanged. No production requests, no upload, no App Review submission.','devices':[]}
for label,device in [('iPhone_6.5',phone)]+([('iPad_13',ipads[-1])] if ipads else []):
 ...
 try:
  run('xcrun','simctl','boot',udid);run('xcrun','simctl','bootstatus',udid,'-b')
  ...
  for number,scene in enumerate(['today','chores','calendar','add','insights'],1):
   ...
   for _ in range(100):
    ...
    time.sleep(1)
   shot=folder/(str(number).zfill(2)+'_'+scene+'.png')
   run('xcrun','simctl','io',udid,'screenshot','--type=png',str(shot))
   ...
   print('Captured',label,scene,record['width'],record['height'],flush=True)
  provenance['devices'].append({'device':device,'folder':label,'captures':records})
 finally:run('xcrun','simctl','shutdown',udid,check=False)
